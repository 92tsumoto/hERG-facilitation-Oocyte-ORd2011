...
void initial_mem()
{
	...
	// ikr2
	ikr2.Txrss=(Number *)calloc(VNMAX,sizeof(Number));
	ikr2.Ttauxr_fast=(Number *)calloc(VNMAX,sizeof(Number));
	ikr2.Ttauxr_slow=(Number *)calloc(VNMAX,sizeof(Number));
	ikr2.TAxr_fast=(Number *)calloc(VNMAX,sizeof(Number));
	ikr2.Trkr=(Number *)calloc(VNMAX,sizeof(Number));
	if( ikr2.Txrss==NULL || ikr2.Ttauxr_fast==NULL || ikr2.Ttauxr_slow==NULL 
		|| ikr2.TAxr_fast==NULL || ikr2.Trkr==NULL ) exit(1);
	// iks
...
void closed_mem()
{
	...
	free(ina.Tmlss); free(ina.Thlss); free(ina.ThlCaMKss);
	...
	free(ikr2.Txrss); free(ikr2.Ttauxr_fast); free(ikr2.Ttauxr_slow); 
	free(ikr2.TAxr_fast); free(ikr2.Trkr);
