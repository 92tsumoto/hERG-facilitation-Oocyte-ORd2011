...
void make_ExpTable()
{
	...
	for(vindex=0;vindex<VNMAX;vindex++){
		...
		// non-facilitation component for ikr 
		...
		// facilitation component for ikr 
		ikr2.Txrss[vindex] = 1.0/(1.0+exp(-(v+8.337+26.6)/6.789));
		ikr2.Ttauxr_fast[vindex] = 12.98+1.0/(0.3652*exp((v-31.66+26.6)/3.869)+4.123E-5*exp(-(v-47.78+26.6)/20.38));
		ikr2.Ttauxr_slow[vindex] = 1.865+1.0/(0.06629*exp((v-34.70+26.6)/7.355)+1.128E-5*exp(-(v-29.74+26.6)/25.94));
		ikr2.TAxr_fast[vindex] = 1.0/(1.0+exp((v+54.81+26.6)/38.21));
		ikr2.Trkr[vindex] = 1.0/(1.0+exp((v+55.0)/75.0))*1.0/(1.0+exp((v-10.0)/30.0));
