...
//		xhplot(LINEATT,0,0,WHITE);
		...
		printf("tt=%lf,ttt=%lf\n",tt,ttt);
		for(ra=0;ra<100;ra++){
			var.block_rate=1.0-(double)ra/100.0;
			var.drug_concentration = var.ic50*pow(1.0/var.block_rate-1.0,1/var.hillc);
			if(var.model_type==0){ // facilitation
				var.fraction_facilitation = 1.0/(1.0+pow((var.drug_concentration/var.fic50),var.fhillc));
			} else {
			...
			while (1){
				...
				//if(var.sswitch==1){	
					apd=ttwoapd - toneapd;
					printf("%d %lf %lf ",var.beat,var.block_rate,time);
					for(w=0;w<NN+1;w++){
						if(w!=NN){
							printf("%10.9lf ",x[w]);
						} else {
				...
				//mouse(&mode,x,dvdt);
				printf("%10.9e %10.9e %10.9e %10.9e\n",v_old,x[0],fabs(v_old-x[0]),fabs(pre_apd90-apd));
				if (fabs(v_old-x[0]) < 1e-5 && fabs(pre_apd90-apd) < 1e-8){ cc++; } else {cc=0;}
				if(cc>30){var.beat=0; cc=0;break;}
				var.beat++;
				pre_apd90=apd;

			} // end for while loop
			fprintf(fp2,"%d %lf %lf ",var.beat,var.block_rate,time);
			for(w=0;w<NN+1;w++){
				if(w!=NN){
					fprintf(fp2,"%10.9lf ",x[w]);
				} else {
					fprintf(fp2,"%10.9lf\n",apd);
				}
			}
